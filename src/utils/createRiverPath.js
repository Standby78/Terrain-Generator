...
const createRiverPath = (terrainArray) => {
    // check if there are adjecent rivers
    const conditionsArray = (mapIndex) => {
        return [
    ...
    // 'flatten' terrain to 0 - 1 (0 === water, 1 === terrain) so we can use A*
    ...
    for (let index = gridPath.length - 1; index > 0; index--) {
        ...
        if (conditionsArray(mapIndex).includes(true)) break;
        newLength++;
    }
